...
namespace Klevu\Search\Test\Integration\Helper\Product\Sync\Price;
...
class ConfigurableProductTest extends TestCase
{
    ...
    public function test_CatalogExcludes_DisplayBoth_SearchExcludes()
    {
        ...
        $parentProduct = $this->getProduct('klevu_configurable_1', $store->getId());
        $product = $this->getProduct('klevu_simple_child_1', $store->getId());
        $currency = $product->getCurrency();

        $klevuProduct = $this->instantiateKlevuProduct();
        $salePrice = $klevuProduct->getSalePriceData($parentProduct, $product, [], $store);
        $startPrice = $klevuProduct->getStartPriceData($parentProduct, $product, [], $store);
        $toPrice = $klevuProduct->getToPriceData($parentProduct, $product, [], $store);
        $price = $klevuProduct->getPriceData($parentProduct, $product, [], $store);
        $groupPrices = $klevuProduct->getGroupPricesData($product, $store);
        $otherPrices = $klevuProduct->getOtherPrices($product, $currency, $store);

        $this->assertSame(49.99, $salePrice, 'Sale Price');
        $this->assertSame(49.99, $startPrice, 'Start Price');
        ...
        foreach ($groupPrices as $groupPrice) {
            ...
            $this->assertSame(80.0, (float)$groupPrice['values'], 'Tier Price Value');
        }
        $this->assertSame('salePrice_-0:80', $otherPrices);
    }
    ...
    public function test_CatalogExcludes_DisplayBoth_SearchIncludes()
    {
        ...
        $this->assertSame(59.99, $salePrice, 'Sale Price');
        $this->assertSame(59.99, $startPrice, 'Start Price');
        $this->assertNull($toPrice, 'To Price');
        $this->assertSame(119.99, $price, 'Price');
        foreach ($groupPrices as $groupPrice) {
            $this->assertSame(96.0, (float)$groupPrice['values'], 'Tier Price Value');
        }
        $this->assertSame('salePrice_-0:96', $otherPrices);
    }
    ...
    public function test_CatalogIncludes_DisplayBoth_SearchExcludes()
    {
        ...
        $this->assertSame(41.66, $salePrice, 'Sale Price');
        $this->assertSame(41.66, $startPrice, 'Start Price');
        $this->assertNull($toPrice, 'To Price');
        $this->assertSame(83.32, $price, 'Price');
        foreach ($groupPrices as $groupPrice) {
            $this->assertSame(66.67, (float)$groupPrice['values'], 'Tier Price Value');
        }
        $this->assertSame('salePrice_-0:66.67', $otherPrices);
    }
