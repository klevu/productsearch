...
namespace Klevu\Search\Test\Integration\Metadata\Block;
...
class CategoryPageOutputTest extends AbstractControllerTestCase
{
     ...
     */
    public function testMetadataJavascriptIsOutputToPageWhenSearchEnabled()
    {
        $this->setupPhp5();
        ...
        $pageMetaMatches = [];
        preg_match(
            '#<script\s*type="text&\#x2F;javascript"\s*id="klevu_page_meta"\s*>\s*.*klevu_page_meta\s*=.*"pageType"\s*:\s*"category".*\s*</script>#',
            $responseBody,
            $pageMetaMatches
        );
        $this->assertNotCount(
