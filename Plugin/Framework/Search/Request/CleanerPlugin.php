...
class CleanerPlugin
{
     ...
     * @see \Klevu\Search\Logger\Logger\PreserveLayout
     * @var bool
     */
    ...
    public function afterClean(MagentoCleaner $subject, $result)
    {
        try {
            ...
            if ($this->klevuHelperConfig->isLandingEnabled() != 1
                || !$this->klevuHelperConfig->isExtensionConfigured()
            ) {
                return $result;
            }
            $this->isKlevuPreserveLogEnabled = $this->klevuHelperConfig->isPreserveLayoutLogEnabled();

            $result = $this->klevuQueryCleanup($result);
        } catch (\Exception $e) {
            $this->klevuHelperData->log(
                LoggerConstants::ZEND_LOG_CRIT,
                sprintf("Klevu_Search_Cleaner::Cleaner ERROR occured %s", $e->getMessage())
            );
        }
    ...
    public function klevuQueryCleanup($requestData)
    {
        ...
        $parentChildIDs = $this->sessionObjectHandler->getData(
            'parentChildIDs_' . $queryScope . '_' . $queryTerm . $categoryFilter
        );
        if (!$parentChildIDs) {
        ...
        if (empty($idList) && $currentEngine === 'mysql') {
            $this->writeToPreserveLayoutLog(
                "searchCleanerPlugin:: MySQL Search Engine is selected and No Ids were found in CleanerPlugin"
            );
        ...
        foreach ($requestData['queries']['quick_search_container']['queryReference'] as $key => $filter) {
            if ($filter['ref'] == 'search') {
                $requestData['queries']['quick_search_container']['queryReference'][$key] = [
                    'clause' => 'must',
                    'ref' => 'klevu_id_search',
                ];
        ...
        $requestData['queries']['klevu_id_search'] = [
        ...
        $this->writeToPreserveLayoutLog(
            ...
            "searchCleanerPlugin:: Request data in CleanerPlugin" . PHP_EOL . print_r($requestDataToArray, true)
        );
